...
export default function PostInteractionBar({ post, comment_enable, onCommentClicked }: PostProps) {
  ...
  const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const buttonName = e.currentTarget.name as string;
  ...
  return (
    <div className="flex gap-2 self-start">
      ...
      <button 
        ...
        className={`text-button flex items-center flex-nowrap gap-1 bg-monochrome-100 py-2 px-2 rounded-[1.5rem] group hover:text-primary-500 transition duration-200`}
      >

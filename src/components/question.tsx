...
const Questionbox = () => {
  ...
  useEffect(() => {
    ...
    updateSlidesPerPage();  // Initialize on first load
    window.addEventListener('resize', updateSlidesPerPage);  // Update on resize
  ...
  const handleMouseUp = () => {
    setIsDragging(false);
    const totalSlides = Math.ceil(mockData.length / slidesPerPage);
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-6 md:px-6 py-6 mb-10">
        <div
        ...
        >
          ...
          >
            {mockData.map((item) => (
              <div
                key={item.id}
                className="flex-shrink-0 px-6 py-6 bg-primary-600 rounded-md text-monochrome-50 h-150"
                style={{
                  minWidth: `calc(${100 / slidesPerPage}% - 1rem)`,
                  flexBasis: `calc(${100 / slidesPerPage}% - 1rem)`,
              ...
              >
                ...
                <div
                  className="mt-4 text-body-large"
          ...
          <div className="flex justify-center gap-2 mt-6 mb-8">
            {Array.from({ length: Math.ceil(mockData.length / slidesPerPage) }).map((_, index) => (
              <button
                key={index}
                onClick={() => setCurrentIndex(index)}
                className={`w-3 h-3 rounded-full ${
                  index === currentIndex ? 'bg-primary-600' : 'bg-monochrome-300'
                }`}
              ></button>
            ))}
          </div>
